...
class UResidualMambaSeq(nn.Module):
    def __init__(self,seq_len:int,mid_ssm_num_s:int,expand_s:int,headdim_s:int,n_levels:int=2):
        ...
        for i in range(self.level):
            ...
            self.downs.append(MambaEncoderForward(seq_len//pow(2,i+1),expand_s,headdim_s))
        '''
        middle parts
        '''
        mid_ssms = []
        for _ in range(mid_ssm_num_s):
            mid_ssms.append(MambaEncoderForward(seq_len//pow(2,n_levels),expand_s,headdim_s))
        ...
        for i in range(n_levels,0,-1):
            ...
            self.ups.append(MambaEncoderForward(seq_len//pow(2,i-1),expand_s,headdim_s))
...
class SIM2GPImputer_inference(nn.Module):
    def __init__(
        self,
        layers,
        seq_len,
